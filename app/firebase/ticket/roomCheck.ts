...
/**
 * 🎯 Fonction utilitaire pour obtenir un résumé rapide des tickets actifs par salle
 * @param restaurantId ID du restaurant
 * @returns Résumé des tickets actifs
 */
export const getActiveTicketsSummary = async (restaurantId: string): Promise<{
  totalActiveTickets: number;
  roomsWithActiveTickets: number;
  tablesWithActiveTickets: number;
  roomDetails: { roomId: string; activeTicketsCount: number; tables: string[] }[];
}> => {
  try {
    // 🚀 Auto-démarrer la synchronisation temps réel si pas encore active
    await ensureTicketsRealtimeSyncStarted(restaurantId);
    ...
    // Import dynamique pour éviter les dépendances circulaires
    const { getRooms } = await import('@/app/firebase/room&table/room');
    
    // Récupérer toutes les salles
    const rooms = await getRooms(restaurantId, true);
    const roomIds = rooms.map(room => room.id!);
    
    // Vérifier toutes les salles en parallèle
    const roomResults = await checkActiveTicketsInMultipleRooms(roomIds, restaurantId);
    
    let totalActiveTickets = 0;
    let roomsWithActiveTickets = 0;
    let tablesWithActiveTickets = 0;
    const roomDetails: { roomId: string; activeTicketsCount: number; tables: string[] }[] = [];
    
    roomResults.forEach((result, roomId) => {
      if (result.hasActiveTickets) {
        roomsWithActiveTickets++;
        totalActiveTickets += result.activeTicketsCount;
        tablesWithActiveTickets += result.activeTablesWithTickets.length;
        
        roomDetails.push({
          roomId,
          activeTicketsCount: result.activeTicketsCount,
          tables: result.activeTablesWithTickets.map(t => `Table ${t.tableNumero}`)
        });
      }
    });
    
    console.log(`📊 Résumé: ${totalActiveTickets} tickets actifs dans ${roomsWithActiveTickets} salles`);
    
    return {
      totalActiveTickets,
      roomsWithActiveTickets,
      tablesWithActiveTickets,
      roomDetails
    };
  } catch (error) {
    console.error('❌ Erreur lors du résumé des tickets actifs:', error);
    throw error;
  }
};

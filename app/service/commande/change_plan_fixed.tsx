...
export default function ChangePlan() {
  ...
  useEffect(() => {
    if (Platform.OS === 'web' && containerRef.current) {
      const handleClick = (event: MouseEvent) => {
        ...
        console.log('Web click:', { clickX, clickY, rect });
        
        moveSelectedTable(clickX, clickY);
      };
  ...
  }, [selectedTableId, containerDimensions]);

  // Move selected table to new position
  const moveSelectedTable = (clickX: number, clickY: number) => {
    ...
    let newX = Math.max(padding, Math.min(maxX, clickX - TABLE_SIZE / 2));
    let newY = Math.max(headerSpace, Math.min(maxY, clickY - TABLE_SIZE / 2));
  ...
  // Handle container press for mobile
  const handleContainerPress = (event: any) => {
    if (Platform.OS === 'web') return; // Web is handled by useEffect
    
    if (!selectedTableId) return;
    
    const clickX = event.nativeEvent.locationX || 0;
    const clickY = event.nativeEvent.locationY || 0;
    
    console.log('Mobile touch:', { clickX, clickY });
